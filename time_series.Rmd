...
fooddiary_cleaned <- fooddiary_cleaned %>% mutate(date_new = as.POSIXct(strptime(datetime, format = "%Y-%m-%d")))

# create date for weeks
# check for different weeknumber dates and replace with the min date
...
# Time Series
```{r}
(decomp_2 <- ggplot(df_all, aes(x = week_date, y = pct_meat)) +
	geom_line() +
...
# Make the plot
(seasonal <- ggplot(df_all, aes(x = month_num, y = pct_meat, group = year)) +
