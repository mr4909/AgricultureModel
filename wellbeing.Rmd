...
final_df_food <- merge(df, temp, by = "hh_ID")
...
pca_data <- pca_data %>% select(-hh_ID)
str(pca_data)
```
...
```{r}
# remove variables
pca_data <- pca_data %>% select(-totalCanStandOwn,-totalCanWalk5Km,-totalCanCarry20L,-bmi)
...
```{r}
factor20 <- fa(pca_data,nfactors = 20,rotate = "oblimin",fm="minres")
print(factor20)
```
...
Using data from df = without food diary information. Data is scaled.
...
# Regression
```{r}
lm.fit <- glm(satisfaction ~ fac_count + visits_healthcenter + visits_bazaar + 
                           ...
                           occupationType + canCarry20L + canWalk5Km + canStandOwn + bmi + 
                           bmiCategory + isChild + pctUnderweight + pctCanCarry20L + 
                           pctCanWalk5Km + pctCanStandOwn, data = df)
summary(lm.fit)
