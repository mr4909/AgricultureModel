...
hh_all <- merge(hh_all, hh_count, by = c("hh_ID","member_1_ID"))
head(hh_all)
```
...
hh_all <- hh_all %>%mutate(pctUnderweight = totalUnderweight/hh_members) %>% select(-totalUnderweight)
...
# # create is underweight variable
# hh_char <- hh_char %>% mutate(isUnderweight = ifelse(bmi < 18.5, 1, 0))
# # create pct or underweight in the household variable
# underweight <- hh_all %>% group_by(hh_ID) %>% tally(isUnderweight)
# hh_all <- merge(hh_all, underweight, by = "hh_ID")
# hh_all <- hh_all %>% mutate(totalUnderweight = n) %>% select(-n)
...
####################################################
# Health Measures, Num of Children, Num Household Members, Has Underweight Members
####################################################
head(hh_char)
...
# select respondent info
hh_respondent_info <- hh_respondent %>% select(hh_ID, sex, age, maritalStatus, literacy, education, attendingCollege, occupationType, bmiCategory)
# merge respondent info with household info
hh_char <- merge(hh_respondent_info, hh_char, by = "hh_ID")
...
facilities <- `20. Facilities_level_1.dta` %>% select(hh_ID, facility_1_ID, submissiondate, facilities, fac_count)
...
facilities2 <- `21. Facilities_facility_repeat_level_2.dta` %>% select(hh_ID, facility_1_ID, fac_mode, fac_dist, time_taken, time_units)
