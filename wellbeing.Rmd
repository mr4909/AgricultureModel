...
df_food_basic <- merge(calories, hh_count, by = "hh_ID")

df_food_basic <- df_food_basic %>%mutate(avg_calories_member = hh_calories_total/hh_members) 

df_food_basic1 <- df_food_basic %>% select(hh_ID, week_number, avg_calories_member)

# remove outliers with custom outliers function (found at top of code)
df_food_basic1$avg_calories_member <- outliers(df_food_basic1$avg_calories_member)

p <- ggplot(df_food_basic1, aes(avg_calories_member, ..count..))
p <- p + geom_histogram(binwidth = 1, colour="white")
p

qplot(x=week_number, y=avg_calories_member,
      data=df_food_basic1, na.rm=TRUE,
      main="Average Caloric Intake of HH Member",
...
fviz_cos2(res.pca, choice = "var", axes = 1:2, top = 30)
