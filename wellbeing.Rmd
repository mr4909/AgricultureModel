...
final_df <- merge(housing_final_data, hh_respondent, by="hh_ID")
...
xyplot(
  ...
  group = food_name_type, data = fooddiary_all,
  type = c("p", "smooth"),        # Show points and smoothed line
  scales = "free"                 # Make panels axis scales independent
  )
```
```{r}
ggplot(fooddiary_all, aes(x=week_number, color=food_name_type)) +
...
############################################
calories <- fooddiary_all %>% group_by(hh_ID) %>% tally(calories_total_grams)
food_entries <- fooddiary_all %>% count(hh_ID)
temp <- merge(calories, food_entries, by = "hh_ID")
temp <- temp %>% mutate(hh_calories_total = n.x, hh_food_entries = n.y) %>% select(-n.x,-n.y)
...
continuous_variables <- final_df %>% select(
                                            -visits_healthcenter,
                                                -visits_bazaar,
                                                -visits_localshop,
                                                -business,
...
pca_data_PCAmixdata <- lapply(pca_data_noNAs, as.numeric)
str(pca_data_PCAmixdata)

pca_data_PCAmixdata <- as.data.frame(pca_data_PCAmixdata)
pca_data_PCAmixdata <- pca_data_PCAmixdata %>% select(-hh_ID)
str(pca_data_PCAmixdata)
```
...
```{r}
# remove variables
pca_data_PCAmixdata <- pca_data_PCAmixdata %>% select(-totalCanStandOwn,-totalCanWalk5Km,-totalCanCarry20L,-bmi)

res.pca <- PCA(pca_data_PCAmixdata, graph = FALSE)
print(res.pca)
```
...
eig.val
```
Q <- quantile(fooddiary_cleaned_week$calories_total_grams, probs=c(.25, .75), na.rm = FALSE)
iqr <- IQR(fooddiary_cleaned_week$calories_total_grams)
up <-  Q[2]+1.5*iqr # Upper Range  
low<- Q[1]-1.5*iqr # Lower Range
fooddiary_cleaned_week_no_outliers_cal <- subset(fooddiary_all, fooddiary_all$calories_total_grams > (Q[1] - 1.5*iqr) & fooddiary_all$calories_total_grams < (Q[2]+1.5*iqr))
summary(fooddiary_cleaned_week_no_outliers_cal$calories_total_grams)nd contributions)
...
ev <- eigen(cor(pca_data_PCAmixdata)) # get eigenvalues
ap <- parallel(subject=nrow(pca_data_PCAmixdata),var=ncol(pca_data_PCAmixdata), rep=100, cent=.05)
nS <- nScree(x=ev$values, aparallel=ap$eigen$qevpea)
plotnScree(nS)
```

Optimal factor levels = 20.
...
```{r}
factor20 <- fa(pca_data_PCAmixdata,nfactors = 20,rotate = "oblimin",fm="minres")
print(factor20)
```
...
print(factor20$loadings,cutoff = 0.4)
...
pca_data_PCAmixdata %>%    # Remove column 5 (Species)
...
library("NbClust")
library(factoextra)
res.nbclust <- pca_data_PCAmixdata %>%
  scale() %>%
...
my_data <- scale(pca_data_PCAmixdata)
fviz_nbclust(my_data, kmeans, method = "gap_stat")
```
```{r}
# Compute k-means
set.seed(123)
km.res <- kmeans(scale(pca_data_PCAmixdata), 3, nstart = 25)

# Visualize
fviz_cluster(km.res, data = pca_data_PCAmixdata,
...
Using data from final_df = without food diary information.
...
continuous_variables <- final_df %>% select(hh_ID,
                                            satisfaction,
                                            age,
                                            bmi,
                                            numChildren,
...
continuous_variables <- data.frame(continuous_variables1,continuous_variables$hh_ID)
continuous_variables <- continuous_variables %>% mutate(hh_ID = continuous_variables.hh_ID) %>% select(-continuous_variables.hh_ID)
df <- merge(continuous_variables, categorical_variables, by = "hh_ID")
...
x <- model.matrix(satisfaction~., train)[,23:51]
...
```{r}
x2=model.matrix(satisfaction~.-1,data=final_df)
